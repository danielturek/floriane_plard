...
data <- list(y = as.numeric(table(Count$year)),
             y1=as.numeric(table(Count$year))[1],
             Nest = as.numeric(table(Spop$year[Spop$S==1]))[1:(nbr.an-1)],
             Rec = aggregate(x =Spop$R[Spop$S==1], by = list(Spop$year[Spop$S==1]), FUN = "sum")$x[1:(nbr.an-1)],
             NSurv=table(Spop$agef[Spop$S==1],Spop$year[Spop$S==1])[,1:(nbr.an-1)],
             NInd=table(Spop$agef,Spop$year)[,1:(nbr.an-1)]
             )

Inits <- list(Rp = 0, Sp = c(0.5,0.5), sigmaS=0.1, sigmaSa=0.1, sigmaR=0.1)

projp <- nimbleModel(code = ipop1Code , name = 'projp' , constants = Consts, data = data, inits = Inits)
...
ipm2_1Code <- nimbleCode({
    ...
    for (u in 1:3){ Sp[u] ~ dnorm(0,0.01) }
    for (u in 1:2){ Rp[u] ~ dnorm(0,0.01) }
    for (u in 1:2){ Ip[u] ~ dnorm(0,0.01) }
    Ipv~ dunif(0, 10)
    ## CHANGE
    ## this is a new prior:
    sd_I ~ dunif(0,100)
...
data2 <- list(y = table(cut(Count$X,breaks=b),Count$year),
              inipop=iniPop2,
              Sdata=Spop$S,
              SX=Spop$X,
              Rdata=Spop$R[Spop$S==1],
              RX=Spop$X[Spop$S==1],
              Idata=Spop$X[Spop$inh==1],
              IX=Spop$PdsMere[Spop$inh==1])
...
### CHANGE
### initial value of Ipv changed from 0 to 1
##Inits2 <- list(Rp = c(0,0),Ip = c(0,0),Ipv=0, Sp = c(0,0.5,0.5))
Inits2 <- list(Rp = c(0,0),Ip = c(0,0),Ipv=1, Sp = c(0,0.5,0.5))

### CHANGE
### add check = FALSE
##mod2 <- nimbleModel(code = ipm2_1Code , name = 'mod2' , constants = Consts2, data = data2, inits = Inits2)
mod2 <- nimbleModel(code = ipm2_1Code , name = 'mod2' , constants = Consts2, data = data2, inits = Inits2, check = FALSE)

nn <- mod2$getNodeNames()
nn <- mod2$getNodeNames(stochOnly = TRUE)
nn <- mod2$getNodeNames(stochOnly = TRUE, includeData = FALSE)
length(nn)
nn
...
dim(MCMCsamplesmod2)
MCMCsamplesmod2[1:50,]

apply(MCMCsamplesmod2[401:1000,],2, mean)
